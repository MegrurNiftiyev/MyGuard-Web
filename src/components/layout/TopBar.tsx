    ...
    <header className="sticky top-4 z-50 w-full max-w-[1440px] mx-auto px-4 sm:px-8 md:pl-24 pointer-events-auto">
      <div className="px-6 py-3 rounded-full bg-surface-container-lowest/85 backdrop-blur-xl border border-outline-variant/70 shadow-xs flex items-center justify-between gap-4 transition-all">
        ...
        <div className="flex items-center gap-3">
          {/* New Chat Session Button (Only visible on /assistant page) */}
          {location.pathname === '/assistant' && (
            <button
              type="button"
              onClick={() => window.dispatchEvent(new Event('trigger-new-chat-session'))}
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-blue-50/90 hover:bg-brand-blue text-brand-blue hover:text-white border border-brand-blue/30 shadow-xs hover:shadow-md transition-all active:scale-95 cursor-pointer flex items-center justify-center shrink-0 group"
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-3 w-80 bg-white/95 backdrop-blur-xl border border-outline-variant/80 rounded-3xl shadow-2xl p-4 z-[100] animate-in fade-in zoom-in-95 duration-150 space-y-3.5">
                ...
                <div className="p-3.5 bg-surface-container-low/70 rounded-2xl space-y-3 border border-outline-variant/40">
                  ...
                  <div className="pt-2.5 border-t border-outline-variant/50 text-xs text-on-surface-variant space-y-2">
